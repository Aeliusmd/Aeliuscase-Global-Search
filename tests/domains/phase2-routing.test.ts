...
// Live bug fixed 2026-08-24, revised 2026-08-25 (case AE0032B). AeliusCase has
// two attorney taxonomies: case-level staff (attorney, supervisor attorney,
// paralegal, coordinator) and party roles (Defense/Applicant/Prior/Referred
// Out/UEF Attorney). "Who is the defense attorney for Thomas Smith vs Matrix?"
// answered "Phoebe Peterson" — the firm's own attorney on the case.
//
// These were briefly routed to getCaseParties, but that endpoint returns only
// partyType/partyName/docs so it could only ever name the firm ("ASDFE"). The
// contact PERSON lives in GetCaseFullDetail's nested case.parties[], now mapped
// to `contacts`, so getCaseFullDetail owns both taxonomies and the system
// prompt decides which field each phrasing reads.
describe('attorney roles all resolve through getCaseFullDetail', () => {
  ...
  for (const q of attorneyQuestions) {
    it(`routes "${q}" to getCaseFullDetail`, () => {
      const { sel } = route(q);
      expect(sel.activeTools).toContain('getCaseFullDetail');
    });
